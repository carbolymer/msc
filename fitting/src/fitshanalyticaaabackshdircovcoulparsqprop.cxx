#include "fitshanalyticaaabackshdircovcoulparsqprop.hxx"
...
double getavk(TGraph *grk, Double_t minb, Double_t maxb)
{
  ...
  for (int iter=bins; iter<bine; iter++) {
    ksum += 1.0*grk->GetY()[iter]*grk->GetX()[iter]*grk->GetX()[iter];
    wsum += 1.0*grk->GetX()[iter]*grk->GetX()[iter];
  }
...
Double_t funfull(Double_t *x, Double_t *par, Double_t *cv00, Double_t *cv20, Double_t *cv22)
{
  ...
  for (int icth=0; icth<BINSC; icth++) {
    ...
    for (int iphi=0; iphi<BINSP; iphi++) {
      ...
      de = (sqrt(0.13957*0.13957 + (Qslp + sqrt(qo2)/2.0)*(Qslp + sqrt(qo2)/2.0) + qs2/4.0 + ql2/4.0) 
        - sqrt(0.13957*0.13957 + (Qslp - sqrt(qo2)/2.0)*(Qslp - sqrt(qo2)/2.0) + qs2/4.0 + ql2/4.0));
      qisq = (qo2/1.0 + qs2/1.0 + ql2/1.0 - de*de);
      
      //      cout << qo2 << " " << qs2 << " " << ql2 << " " << de << " " << qisq << endl;
      qinv = sqrt(TMath::Abs(qisq));

      kavval = getavk(grkcoul, qinv/2.0-0.02*Qslp, qinv/2.0+0.02*Qslp);
...
//   Double_t c00val = 0;
  ...
  for (int iter=binb; iter<bine; iter++) {
    ...
    xfi[iter] = qv;
    y00fi[iter] = tv00*ng;
    y20fi[iter] = (tv20*ng+c20lam*exp(-c20rad*c20rad*qv*qv)+c20hval*exp(-(qv-c20mval)*(qv-c20mval)*c20wval*c20wval));
    y22fi[iter] = tv22*ng;
...
//   part[18] = fitter->GetParameter(19);
  ...
  TGraph *grfi00 = new TGraph(bine-binb+1, xfi, y00fi);
  TGraph *grfi20 = new TGraph(bine-binb+1, xfi, y20fi);
  TGraph *grfi22 = new TGraph(bine-binb+1, xfi, y22fi);

  grfi00->SetName("grfi00");
  grfi20->SetName("grfi20");
  grfi22->SetName("grfi22");
  ...
  c00->GetXaxis()->SetRangeUser(0,0.4);
  c00->GetYaxis()->SetTitleSize(0.07);
  c00->GetYaxis()->SetLabelSize(0.05);
  c00->SetMaximum(2.13);
  ...
  c20->GetXaxis()->SetRangeUser(0,0.4);
  c20->GetYaxis()->SetTitleSize(0.07);
  ...
  c22->GetXaxis()->SetLabelSize(0.05);
  c22->GetXaxis()->SetRangeUser(0,0.4);
  c22->GetYaxis()->SetTitleSize(0.07);
